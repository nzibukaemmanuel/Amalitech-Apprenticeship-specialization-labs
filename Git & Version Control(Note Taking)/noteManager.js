...
/** A note is only worth importing if it has a real title and a content string. */
function isValidImportedNote(n) {
  return Boolean(
    n && typeof n === 'object' &&
    typeof n.title === 'string' && n.title.trim().length >= 3 &&
    typeof n.content === 'string' &&
    (n.tags === undefined || typeof n.tags === 'string' || Array.isArray(n.tags))
  );
}

/** Case-insensitive title+content fingerprint, so the same note re-imported
 *  under a new id (or exported/re-imported round-trip) still counts as a dup. */
function noteSignature(n) {
  return `${String(n.title).trim().toLowerCase()}::${String(n.content).trim().toLowerCase()}`;
}

/**
 * Merge imported notes into the existing collection (bonus). Skips anything
 * that doesn't look like a note, and skips anything that's already present
 * (matched by id, then by title+content) so re-importing the same export
 * — or an export that overlaps the current notes — never creates duplicates.
 */
export const mergeIn = (importedNotes) => {
  if (!Array.isArray(importedNotes)) return { added: 0, skipped: 0 };

  const existingIds = new Set(notes.map((n) => n.id));
  const existingSignatures = new Set(notes.map((n) => noteSignature(n)));
  const additions = [];
  let skipped = 0;
  let orderCounter = Date.now();

  importedNotes.forEach((raw) => {
    if (!isValidImportedNote(raw)) { skipped++; return; }

    const signature = noteSignature(raw);
    if ((raw.id && existingIds.has(raw.id)) || existingSignatures.has(signature)) {
      skipped++;
      return;
    }

    const note = new Note(raw.title, raw.content, raw.tags || []);
    if (typeof raw.archived === 'boolean') note.archived = raw.archived;
    if (raw.location && typeof raw.location === 'object') note.location = raw.location;
