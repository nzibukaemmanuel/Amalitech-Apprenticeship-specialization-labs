...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const countCharacters = text => (
    excludeSpacesBox.checked ? text.replace(/\s/g, '') .length : text.length
  );
  ...
  const calcLetterDensity = text => {
    ...
    for (const rawChar of loweredText) {
      if (/[a-z]/.test(rawChar)) {
    ...
    const maxCount = countValues.length ? countValues.reduce((highest, count) => Math.max(highest, count), 0) : 0;
  ...
  const render = () => {
    ...
    if (lineText.length < 2) lineText = '0' + lineText;
  ...
  enableLimitBox.addEventListener('change', () => {
    ...
    if (enableLimitBox.checked) limitInput.focus();
    render();
  });
